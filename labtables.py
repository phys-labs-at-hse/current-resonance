...
def uscatter(x, y, label=None, add_line=False, check_slope=False, student_t=3):
    """Scatter-plot arrays of uncertainties.ufloats, adding errorbars.

    Also works if x, y are lists or numpy.arrays. In this case, zero
    error is assumed.

    If add_line is True, also draw a linear regression line calculated
    with numpy.polyfit and return slope, intercept and slope error.
    Slope error is calculated as student_t * std, where std is taken from
    numpy.polyfit output.

    If add_line and check_slope are both True, draw auxilary gray lines
    illustrating the slope error.

    label is passed to the matplotlib.pyplot.errorbar.
    """
    ...
    plt.errorbar(x_nom, y_nom, xerr=x_std, yerr=y_std, fmt='o', label=label)
    if add_line:
        (slope, intercept), cov = np.polyfit(x_nom, y_nom, 1, cov=True)
        error = student_t * np.sqrt(cov[0][0])
        # print(f'Plotting line with slope = {slope}, intercept = {intercept}')
        plt.plot(x_nom, slope * x_nom + intercept,
                 color=plt.gca().lines[-1].get_color())
        if check_slope:
            plt.plot(x_nom, (slope - error) * (x_nom - np.mean(x_nom)) +
                            np.mean(y_nom), '--', color='gray')
            plt.plot(x_nom, (slope + error) * (x_nom - np.mean(x_nom)) +
                            np.mean(y_nom), '--', color='gray')
